...
public class Swimmer extends Thread {
	...
	//Constructor
	Swimmer( int ID, int t, PeopleLocation loc, FinishCounter f, int speed, SwimStroke s, AtomicInteger correctOrder, CyclicBarrier startBarrier, CountDownLatch startingLatch) {
		this.swimStroke = s;
	...
	public void run() {
		try {
			...
			synchronized (correctOrder){
				while (!(swimStroke.order - 1 == correctOrder.get()))wait();
				
				dive();
				swimRace();
				correctOrder.set(correctOrder.get() + 1);
		}
